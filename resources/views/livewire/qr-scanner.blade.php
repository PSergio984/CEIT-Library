    ...
    @if ($isScanning)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm"
            x-data="{
                ...
                async selectMode(mode) {
                    ...
                    if (mode === 'camera') {
                        // Enumerate cameras
                        if (window.getAvailableCameras) {
                            this.cameras = await window.getAvailableCameras();
                            console.log('Available cameras:', this.cameras);
                        }
                        
                        setTimeout(async () => {
                            if (window.initCameraScanner) {
                                this.currentCameraId = await window.initCameraScanner();
                            }
                        }, 100);
                    } else if (mode === 'file') {
                        setTimeout(() => {
                ...
                async switchCamera(cameraId) {
                    ...
                    if (window.initCameraScanner) {
                        await window.initCameraScanner(cameraId);
                    }
                },
                async flipCamera() {
                    if (this.cameras.length < 2) return;
                    // Find current camera index or default to 0
                    ...
                    await this.switchCamera(this.cameras[nextIndex].id);
                },
                reset() {
                    this.scanMode = null;
                    this.cameras = [];
            ...
            <div class="bg-base-100 rounded-2xl shadow-2xl p-4 sm:p-6 mx-4 max-h-[95vh] overflow-y-auto"
                ...
                <div x-show="scanMode === 'camera'" x-transition class="relative">
                    ...
                    {{-- Scanner Status & Camera Selection --}}
                    <div class="mb-4 space-y-2">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between bg-base-200 rounded-lg p-3 gap-3">
                            <div class="flex items-center justify-between flex-1">
                                ...
                                {{-- Camera Switching UI --}}
                                <div class="flex items-center gap-2">
                                    {{-- Flip button for exactly 2 cameras (common on mobile) --}}
                                    <template x-if="cameras.length === 2">
                                        <button @click="flipCamera()" class="btn btn-sm btn-circle btn-ghost tooltip" data-tip="Flip Camera">
                                            <x-mary-icon name="o-arrow-path" class="w-5 h-5" />
                                        </button>
                                    </template>

                                    {{-- Dropdown for more than 2 cameras --}}
                                    <template x-if="cameras.length > 2">
                                        <select 
                                            class="select select-xs select-bordered w-32"
                                            x-model="currentCameraId"
        ...
        @script
            <script>
                 ...
                 */
                window.initCameraScanner = async function(preferredCameraId = null) {
                    console.log('initCameraScanner called', { preferredCameraId });
                    ...
                    try {
                        ...
                        const cameraParam = preferredCameraId ? preferredCameraId : { facingMode: "environment" };
                        ...
                        // Return the actual camera ID being used (Html5Qrcode doesn't return it directly from start, 
                        // but we can try to find it if we passed it, or just return true)
                        return preferredCameraId || true;
                    } catch (error) {
