...
<div class="min-h-screen bg-base-200/30 p-4 md:p-6">
    ...
    <x-mary-modal wire:model="isReturnQrModalOpen" title="Return QR Code" box-class="max-w-lg">
        @if($returnQrCodeDataUri)
            ...
            <x-slot:actions>
                @if($returnQrTransactionId)
                    <button 
                        wire:click="downloadReturnQr"
                        wire:loading.attr="disabled"
                        wire:target="downloadReturnQr"
                        class="btn btn-success gap-2">
                        <span wire:loading.remove wire:target="downloadReturnQr" class="flex items-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3" />
                            </svg>
                            Download
                        </span>
                        <span wire:loading wire:target="downloadReturnQr" class="flex items-center gap-2">
