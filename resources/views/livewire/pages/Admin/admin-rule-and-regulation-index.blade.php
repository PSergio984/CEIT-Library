<div>
    ...
    <div class="mt-3 mb-4">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <x-mary-select label="Header" :options="$headers_list" option-label="title" option-value="id" placeholder="All headers"
                wire:model.live.debounce.100ms="filterHeaderId" clearable class="w-full sm:w-64" />
            <input type="text" wire:model.live.debounce.100ms="search" placeholder="Search rules..."
    ...
    @if ($this->rules()->isNotEmpty())
        <x-mary-table :headers="$headers" :rows="$this->rules()" :sort-by="$sortBy" with-pagination :per-page="$perPage"
            ...
            @scope('actions', $rule)
                ...
                @else
                    <span class="text-sm text-base-content/70 italic">View Only</span>
                @endif
            @endscope
        </x-mary-table>
    @else
